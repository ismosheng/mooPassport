...
const eventLabels = {
  ...
  'user.login.succeeded': '通行证账号登录成功',
  'user.login.failed': '通行证账号登录失败',
  'user.logout': '用户退出登录',
  ...
  'oauth.authorization.request_pushed': '应用创建授权请求',
  'oauth.authorization.approved': '用户同意应用授权',
  'oauth.authorization.denied': '用户拒绝应用授权',
  'oauth.consent.revoked': '用户撤销应用授权',
  'oauth.token.issued': '应用登录成功（Token 已签发）',
...
const columns = [
  ...
  { title:'HTTP 请求 ID',key:'request_id',minWidth:180,ellipsis:{tooltip:true},render:(row)=>row.request_id||'-' },
