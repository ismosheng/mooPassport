...
- 方法保持简短，不承载业务分支。
- Controller 由容器复用，必须保持无状态；禁止把 Request、用户或本次请求数据
  保存到实例属性。只允许保存通过构造函数注入的无状态 Service。
...
- 项目自有代码的说明性注释统一使用中文；类名、协议术语、字段名、错误码以及
  PHPDoc 类型标注保持其原始英文形式。第三方许可证头不得修改。
...
```php
// 令牌族中任一令牌被重复使用都表示可能已泄露，因此撤销整个令牌族。
